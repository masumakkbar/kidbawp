...
<div class="tutor-pagination-wrapper-replaceable">
	<h3 class="tutor-fs-5 tutor-fw-bold tutor-color-black tutor-mb-24">
		<?php
			...
			echo esc_html( $review_title, 'kidba' );
		?>
	</h3>

	<?php if(! is_array( $reviews ) || ! count( $reviews )): ?>
		<?php tutor_utils()->tutor_empty_state(__('No Review Yet', 'kidba')); ?>
	<?php else: ?>
				...
				<div class="tutor-row tutor-gx-xl-5 tutor-align-center">
					<div class="tutor-col-lg-auto tutor-text-center tutor-mb-16 tutor-mb-lg-0">
						...
						<div class="tutor-fs-6 tutor-color-secondary tutor-mt-12">
							<?php esc_html_e( 'Total ', 'kidba' ); ?>
							<?php echo $reviews_total; ?>
							<?php echo esc_html( _n( ' Rating', ' Ratings', count( $reviews ), 'kidba' ) ); ?>
						</div>
					</div>
							...
							<?php foreach ( $rating->count_by_value as $key => $value ) : ?>
								...
								<div class="tutor-row tutor-align-center tutor-review-summary-rating">
									...
									<div class="tutor-col-4 tutor-col-lg-3">
										<span class="tutor-fs-6 tutor-color-secondary"><?php echo $value . ' ' . ( $value > 1 ? __( 'Ratings', 'kidba' ) : __( 'Rating', 'kidba' ) ); ?></span>
									</div>
								</div>
							<?php endforeach; ?>
						</div>
	...
	<div class="tutor-row tutor-mt-40 tutor-mb-20">
		<div class="tutor-col">
			<?php if($is_enrolled): ?>
				<button class="tutor-btn def-btn border-0 write-course-review-link-btn">
					<i class="tutor-icon-star-line tutor-mr-8"></i>
					<?php
						$is_new = !$my_rating || empty($my_rating->rating) || empty($my_rating->comment_content);
						$is_new ? _e('Write a review', 'kidba') : _e('Edit review', 'kidba');
					?>
				</button>
			<?php endif; ?>
		</div>
		...
		<div class="tutor-col-auto">
			<?php
				$pagination_data = array(
					...
					'layout'	  => array(
						'type' => 'load_more',
						'load_more_text' => __('Load More', 'kidba')
					),
					'ajax'		  => array(
			...
			<form method="post">
                <div class="contact-form">
                    ...
                    <div class="contact-col">
                        <textarea name="review" placeholder="<?php _e('write a review', 'kidba'); ?>"><?php echo stripslashes($my_rating ? $my_rating->comment_content : ''); ?></textarea>
                    </div>
                    <div class="tutor-form-group">
                        <button type="submit" class="def-btn  border-0">
                            <?php _e('Submit Review', 'kidba'); ?>
                        </button>
                    </div>
                </div>
			</form>
